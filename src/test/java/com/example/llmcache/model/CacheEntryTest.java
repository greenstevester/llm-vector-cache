...
class CacheEntryTest {
    ...
    void shouldCreateCacheEntryWithAllArgsConstructor() {
        ...
        // When
        CacheEntry entry = new CacheEntry(prompt, response, embedding, timestamp, metadata, id);

        // Then
        assertEquals(prompt, entry.getPrompt());
        assertEquals(response, entry.getResponse());
        assertEquals(embedding, entry.getEmbedding());
    ...
    void shouldGenerateConsistentIdFromPrompt() {
        ...
        // When
        CacheEntry entry1 = new CacheEntry(prompt, response, embedding);
        CacheEntry entry2 = new CacheEntry(prompt, response, embedding);
    ...
    void shouldHaveDifferentIdsForDifferentPrompts() {
        ...
        // When
        CacheEntry entry1 = new CacheEntry("Prompt 1", "Response 1", embedding);
        CacheEntry entry2 = new CacheEntry("Prompt 2", "Response 2", embedding);
    ...
    @Test
    void shouldHandleNullEmbeddingInConstructor() {
        // When
    ...
    void shouldHandleVeryLongPromptsForIdGeneration() {
        ...
        // When
        CacheEntry entry = new CacheEntry(longPrompt, response, embedding);
